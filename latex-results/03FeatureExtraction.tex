...
practitioners are often empirically interested in person-level
...
attitudes towards migrants or do they vary across the measurement period?'\,'
...
stay in a depressed mood for several measurements?'\,'
...
slope provides the average change from one time point \(t\) to the next
across all measurements \(x\) of a time series \(X\) of participant
\(i\) and variable \(j\).
