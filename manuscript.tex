...
Importantly, we show that feature-based clustering is not only approachable but provides interpretable and transparent insights about the grouped patterns. For our example of migration experiences, the method was useful to discern adaptive from more stressful experiences and helped to contextualize divergent experiences. We were able to show which variables and which dynamic features were most impactful in discerning the trajectory clusters and were even able to further contextualize the results with out-of-feature comparisons (see, e.g., \fgrref{fig:clusterFeatVar}). 
...
For instance, given that the approach does not assume the stationarity restrictions of many model-based approaches, future research may want to integrate the research on capturing nonlinear trends from the shape-based approaches into the time series feature world. Bicoherence metrics, polynomial-, and differential equation parameters may be used to capture the type and structure of nonlinear developments \citep[e.g.,][]{caro-martin2018, mayor2022}. All of which would add to the under-studied trend features of ESM data.

Similarly, iterative or embedded approaches have a number of upsides, such as extracting a cluster-level model that captures the prototypical patterns of the group \citep[e.g.,][]{denteuling2021, lane2019}. To provide similar functionality, researchers may evaluate the utility of using recurrent neural networks, such as long short-term memory (LSTM) models to generate cluster-level trajectories \citep[e.g., by training the LSTM based on the feature matrix and the original time series as the target; e.g., see][]{fraley2002, nagin1999}. Alternatively, rule-based classifiers could extract rule sets that narrow down the conditions under which a given feature set is assigned to a specific cluster \citep[][]{benard2019}. 
