...
The main aim of time series clustering is to inductively group participants in such a way that the dynamics of some variable(s) have maximum similarity for individuals in the same group, and minimum similarity for individuals in different groups \citep[e.g.,][]{liao2005}. In practice, this means time series clustering critically depends on identifying similarities or dissimilarities in time series and then accurately distinguishing different groups (REF).

1. phenomena and principle detection
    a. differences between clusters
    b. patterns within clusters (\~ early warning signals?)
2. 

Time series clustering has a number of conceptual use cases with psychological data. Prime among them is the ability to reduce the time, variable, and person complexity by extracting structure between participants. These reduction and structuring qualities can be essential in detecting phenomena and extracting more abstract functional principles \citep[][]{eronen2021a}. 
These phenomena and principles can be (1) meaningful differences between clusters, as well (2) important patterns, trends, and relationships within clusters \citep[e.g.,][]{schrodt2000}. 

Once distinct groups and patterns have been identified, researchers can examine the extent to which these within-group and between-group structures are associated with other variables of interest, such as personality traits, demographic characteristics, or other psychological constructs (REFERENCE).

Such meaningful psychological principles and phenomena have often been recognized as being imperative for building robust theories and real-world interventions \citep[e.g.,][]{borsboom2021, eronen2020}. 
Additionally, time series clustering has been used to inform the development of targeted interventions and therapies for individuals, for example, with mood disorders and other psychological conditions (REFERENCE). 
