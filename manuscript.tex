...
% Title and header
\title{Describe and Explore: Using feature-based time series clustering to understand modern intensive longitudinal data}
...
Recent years have seen a striking increase in the number and variety of research studies that follow participants' everyday experiences and collect real-world psychological time series \citep[e.g., experience sampling method (ESM),][; also see \fgrref{fig:ScopusEsm}]{hamaker2017}. These extensive longitudinal data sets come with new forms of heterogeneity, where researchers have to consider differences across large numbers of participants, time points, and variables \citep[e.g.,][]{cattell1966, Monden2015}. Oftentimes, however, researchers are interested in precisely this complexity and wish to understand how people differ in their temporal developments across a number of distinct variables \citep[i.e., understand the structure of between-subject differences of within-participant dynamics; e.g.,][]{ernst2021}. One relatively recent addition to the toolbox of ESM researchers has been time series clustering --- the idea of grouping participants based on the characteristics of their time series dynamics \citep{ariens2020}.

Clustering holds a number of important prospects as it seeks to organize data into groups that maximizes differences between groups while minimizing differences within the groups. For time series clustering this means that a participant's dynamics of some variable(s) have maximum similarity with the dynamics of other individuals in the same group, and minimum similarity with individuals in other groups \citep[e.g.,][]{liao2005}. As such, time series clustering has a number of methodological and empirical use cases. Methodologically, comparing person-specific dynamics between participants allows researchers to investigate whether processes on the individual level generalize to the interindividual level \citep[i.e., ergodicity; e.g.,][]{adolf2019, molenaar2004}. Similarly, clusters might allow researchers to identify cases where the meaning of measurements might have changed over time \citep[i.e., measurement invariance; e.g., ][]{adolf2014}. For a more detailed of these benefits, see \citet{ernst2021}. 

benefits of exploratory separation
\begin{itemize}[nosep]
    \item with extremely large data sets models can be estimated on a group level rather than for every individual (REFERENCE for TS clustering as preparation step)
    \item describe and explore data with reducing complexity: by extracting structure between participants
    \item phenomena detection \citep[i.e., extract abstract functional principles from phenomena][]{eronen2021a}. Important for building theories and practical interventions.
    \item check of or avoid ergodicity \citep{adolf2019, molenaar2004} and measurement invariance \citep{adolf2014}
\end{itemize}
...
Thus far the most common clustering approach for ESM data has been to group individuals based on their model parameters \citep[e.g., intercepts and slopes from vector autoregression models (VAR),][]{ariens2020}. While these model parameters are familiar to researchers in the field, the model-based approach has a number of important shortcomings. Computationally, the number of parameters compounds with increases in variables and lag orders, which tends to result in efficiency and accuracy reduction for most clustering algorithms and is sometimes referred to as the dimensionality curse \citep{altman2018}\footnote{As an illustration of this scalability issue, consider a VAR model of 8 variables that includes the first three lags (i.e., VAR(3)). For each of the 8 variables we would have 25 parameters --- one intercept and 24 slopes (8 variables * 3 lags each). That compounds to a total of 200 parameters per person (8 variables * 25 parameters).}. But more importantly, using the model parameters means that the clustering is restricted by the assumptions and limitations of the statistical model. To take the VAR model as an example, the model assumes that the time intervals between measurements are identical (i.e., equidistant measurements), it does not allow for missing values, and it assumes that the means as well variances of the variables do not change over time \citep[i.e., stationarity assumption;][]{lutkepohl2005}. 

These shortcomings however stand in sharp contrast with common research interests in the field and the types of data researchers commonly collect. 
Detail on multidimensional nonequidistant structurally missing nonlinear trends. 

we thus need a way of dealing with these types of data. 
While some of these issues can by avoided by transforming the data \citep[e.g., detrending;][]{salles2019} or switching to different time series models \citep[e.g., time-varying VAR;][]{bringmann2018b}, a more flexible alternative would be to expand the clustering inputs to other time series characteristics of interest beyond model parameters. 

Importantly, model intercepts and slopes are not the only way in which researchers conceptualize and describe time series. 
...
Additionally, ``scalability problems, and can lead to performance reductions when the clusters are close to each other''
...
Two important developments within the psychological sciences have been a shift towards more comprehensive assessments of psychological phenomena and a growing contextualization of intensive longitudinal real-life data. Multivariate experience-sampling approaches allow researchers to test theories more exhaustively and help build a more embedded understanding of human experience processes. However, such data also deviates from past intensive longitudinal data sets that have focused on psychological states that are more homogeneous in their developmental characteristics. It, thus, makes sense to briefly consider which challenges modern ESM data introduce and what qualities a present-day clustering analysis should possess. We particularly highlight issues of dimensionality, missingness, and time scales within contemporary ESM data. 

Concerning dimensionality issues, especially more abstract psychological experiences often need a wider variety of measurements to be captured adequately. Today, few clinical conditions are captured with a single symptom measure, emotions are rarely assessed in isolation, and socio-cultural experiences are now widely considered to be multimodal. This also means that modern analysis techniques increasingly need be able to accommodate an increased focus on multivariate developments. The recent rise in psychological network models has highlighted this need for more multivariate outcome measurements \citep[e.g.,][]{decancq2013}. Beyond the capability to assess multivariate developments, an increased number of variables also heightens the computational load for model estimations and, especially when the number of variables and measurements compounds, models may not converge --- an issue sometimes referred to as the dimensionality curse. A modern descriptive and exploratory analysis technique should consequently be able to summarize and structure multivariate phenomena without running into computational load issues.

The push for more contextualized data also presents two key issues with missing data. More event-based assessments tend to (1) encounter issues when models require time intervals between measurements to be identical (i.e., equidistant measurements) and an increased focus on context-specific phenomena has (2) lead to issues when data is only available under certain conditions, including event-based questions (i.e., structurally missing data). Smaller issues of non-equidistant data can be avoided with transformations \citep[e.g., dynamic time warping,][]{berndt1994} or newer modeling procedures \citep[e.g., continuous-time models][]{dehaan-rietdijk2017} but for many analyses, including many cluster approaches, non-equidistant measurements remain a prevalent issue. Structural missingness remains an even more strenuous challenge. Structural missingness occurs when data is missing because it logically cannot be collected. It would not be sensible to measure the quality of an event for participants that did not experience the event. This type of missingness particular becomes an issue when researchers are interested in how two concepts co-develop and one of the concepts has structural missingness under certain conditions. The most common practice for structurally missing data is to either exclude the variable or any measurement that has no structurally missing data \citep[e.g.,][]{lavori2008}\footnote{This is the case because the most commonly used models require complete data \citep{schafer2002} and structurally missing data cannot be imputed as it logically does not exist \citep[e.g.,][]{lavori2008}.} --- neither option suits a research question that includes a developmental relationship between the two variables. In short, adequate descriptive approaches should be able to deal with non-equidistant and structurally missing data in order to address modern ESM data.

Finally, new approaches to ESM data have highlighted two main issues with how temporal developments are conceptualized. Researchers are (1) increasingly interested in how mean-levels of variables change over time, often in a non-linear fashion (i.e., non-stationary), and (2) there is increased interest in relationships between variables that change on fundamentally different time scales. Many of the commonly used analyses for ESM data assume that means and variances are stable over time\footnote{These models include the popular vector autoregressive models, dynamic structural equation models, autoregressive integrated moving average models, and cross-lagged panel analyses. Notable time-varying adaption include \citet{haslbeck2021a}.}. While several trend modeling methodologies exist (e.g., spline regression models, latent growth curve modeling), their availability is limited and they seldomly extend into the descriptive realm. Moreover, among the approaches that allow for means to change, even fewer allow for substantially different time scales between variables, which can be essential in understanding how daily experiences relate to slower psychological phenomena that develop over days, weeks, and months \citep[e.g., see][]{Ram2014}. A modern descriptive and exploratory methodology, including a clustering approach, should thus be able to flexibly capture and compare non-stationary and non-linear trends, also across different time scales.
...
TS feature clustering steps: \citep{rasanen2009, wang2006}
