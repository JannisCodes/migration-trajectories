...
Feature-based time series clustering broadly speaking means that important features from the time series data are extracted (Ratanamahatana & Keogh, 2004). The extracted features can then be used as inputs to a clustering algorithm such as k-means or hierarchical clustering to group the time series into clusters (Xu et al., 2013).

Feature-based time series clustering has been a valued tool in other empirical fields, including analyses of astronomical, meteorological, and aviation pathways, biological and medical developments, as well as energy and finance patterns \citep{Aghabozorgi2015}. 

As an illustration of the flexibility of this approach, a commonly used software package for this approach allows users to extract a total of 794 time series features \citep[][]{christ2018}. 
...
\paragraph{Central Tendency} Familiar statistics from probability theory, the two features sit at the heart of many fundamental psychological questions. ``Are some people happier than others?'' (i.e., difference in central tendency). Mathematically, the central tendency has parametric and robust options to choose from (e.g., parametric: mean; robust: median).

\paragraph{Variability} "Are people settled in their attitudes towards migrants or do they fluctuate over time?'' (i.e., difference in variability). parametric: standard deviation, robust: median absolute deviation.

\paragraph{(In)stability} Stability captures the average change between two consecutive measurements. Do changes tend to be slow and gradual or fast and abrupt? Mathematically, (in)stability measurements often look at the average change between two consecutive measurements \citep[e.g., mean absolute change; e.g.,][]{barandas2020}. 

\paragraph{Self-similarity} Self-similarity describes how much a measurement carries over to future measurements. There are two main ways in which experiences tend to be connected to future experiences --- resistance to change and seasonality. Both forms of self-similarity relate to different types of research questions. ``Do patients stay in a depressed mood for several measurements?'' (i.e., inertia), and ``Do participants drink more alcohol on Fridays?'' (i.e., periodicity). Both forms of self-similarity are often measured using autocorrelations with either a lag-1 (inertia) or the lag of the seasonality (e.g., seven days). Periodicity can additionally be captured using wave patterns \citep[e.g., fourier or wavelet transformation; e.g.,][]{mayor2022}.
