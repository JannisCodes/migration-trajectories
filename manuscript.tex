...
\paragraph{Variability} Variability, also referred to as dispersion or spread, captures to the degree to which a set of data differs from the central tendency \citep{weisberg1992}. In time series analyses, variability is important because it provides information about the distribution and diversity of the data. "Are people settled in their attitudes towards migrants or do they fluctuate over time?'' (i.e., difference in variability). 
There are several measures of variability, including the range, standard deviation, and variance (Hopkins, 2020). The appropriate measure to use depends on the specific characteristics of the data being analyzed. parametric: standard deviation, robust: median absolute deviation.

``Capturing variability may not just be helpful in identifying a worse psychological state or an inadequate emotion regulation strategy. It may also predict future course of symptoms. Affective variability at baseline predicted the recurrence of depressive symptoms over a three-year period in remitted depressed patients \citep{timm2017}. Similarly, elevated temporal auto-correlation and variance, as well as increased correlation between emotions, increased the probability for an upcoming shift between a normal and a depressed state \citep{vandeleemput2014}. ''\citep{myin-germeys2018}

EWS: anomalous variance \citep{wichers2019}

increase in variance \citep{scheffer2009}

\paragraph{(In)stability} Stability captures the average change between two consecutive measurements. While instability is conceptually related to the variability feature, variability does not take into account temporal dependency, whereas instability looks at the `jumpy-ness' of the data over time. In other words, instability reflects the fluctuation or inconsistency in a time series data over time, while variability reflects the range or diversity of values in a time series data \citep{trull2008}. For example, if a person has rapid and extreme changes in mood their mood is highly unstable, while if a person's mood responses span a wide range over the entire study period, their mood is highly variable \citep{jahng2008}. Common research questions with psychological time series might, thus, include: ``Do changes tend to be slow and gradual or fast and abrupt?" Mathematically, (in)stability measurements often look at the average change between two consecutive measurements \citep[e.g., mean absolute change; e.g.,][]{ebner-priemer2009,barandas2020}. 

\paragraph{Self-similarity} Self-similarity describes how much a measurement carries over to future measurements. There are two main ways in which experiences tend to be connected to future experiences --- resistance to change and seasonality. Both forms of self-similarity relate to different types of research questions.

Resistance to change is the self-similarity of a measurement with its next measurement and is often referred to as inertia \citep{REF}. ``Do patients stay in a depressed mood for several measurements?'' 
characterize psychological maladjustment \citep{kuppens2010} and can even predict the initial onset of depression \citep{kuppens2012}.
Mathematically, inertia is often measured using autocorrelations or autoregressions with a lag-1.

Seasonality is the self-similarity of a measurement that returns in a cyclical manner and is often also referred to as periodicity \citep{REF}. ``Do participants drink more alcohol on Fridays?''. Both forms of self-similarity are often measured using autocorrelations with either a lag-1 (inertia) or the lag of the seasonality (e.g., seven days). Periodicity can additionally be captured using wave patterns \citep[e.g., fourier or wavelet transformation; e.g.,][]{mayor2022}.
