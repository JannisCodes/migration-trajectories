...
FitT3Main <- T3fitpartitioning(
  ...
  K = t3Main$H, 
  renormmode = 0,
  laba = paste("PP", 1:nrow(dtMainImpWide), sep = "_"), 
...
rrcov3way::Tucker3(
  X = dtMainImpWide, 
  P = nrow(dtMainImpWide), # Nr. ppt.s (A)
  Q = length(names(dtPcaS123Imp)[!names(dtPcaS123Imp) %in% idVars]), # Nr. variables (B)
  R = length(unique(dtPcaS123Imp$TIDnum)), # Nr. time points (C)
