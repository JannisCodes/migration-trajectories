...
varNames %>%
  ...
  kbl(.,
      ...
      align = c("l", "l", rep("c", ncol(.)-2)),
      digits=2,
      caption = "Full Analysis: Variables by Interaction Types") %>%
  kable_classic(
    full_width = F,
    lightable_options = "hover",
    html_font = "Cambria"
...
scaled_data <- featFull2$featuresImpZMat
...
set.seed(123)
fviz_nbclust(scaled_data, kmeans, nstart = 50,  method = "gap_stat", nboot = 500)+
...
as.data.frame(kmeansOut$cluster)
...
eps_plot %>% abline(h = 13, lty = 2)

eps <- 13
minPts <- 3

res.fpc <- fpc::dbscan(scaled_data, eps=eps, MinPts=minPts, scale=FALSE, method="raw")
