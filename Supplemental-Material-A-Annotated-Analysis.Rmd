...
dtS1_osf <- dtS1$full %>%
  select(all_of(export_vars)) %>%
  mutate(EvDayDiscr.post = rowMeans(across(starts_with("EvDayDiscr")), na.rm = TRUE),
         study = "S1") %>%
  select(-starts_with("EvDayDiscr0")) 
...
saveRDS(dt_osf, file="data/osf_mini.Rda")
```
...
```{r export cleaned and reduced datasets}
# save cleaned 
save(dtS1, dtS2, dtS3, file = "data/s123Full.RData")

# save reduced
save(list = ls(pattern = "Red$"), file = "data/s123Red.RData")
```
...
We included 12 variables that were available across all three studies and captured information about the participant's interactions, as well as cognitive-, emotional-, and motivational self in relationship with the majority group. We chose these two aspects in particular because (1) the interaction-specific information exemplified the structural missingness issue of modern ESM data and (2) the motivational, emotional, and cognitive experience offered a diverse conceptualization of migration experience (beyond behavioral measurements) that is becoming more common in the literature [@Kreienkamp2022d].
...
```{r variable names by response type and study}
varNames %>%
  filter(
    pca != 0,
...
# apply(dtS1Red, 2, sum(is.na(.))/nrow(.))
# 
# your_dataframe %>%
#   group_by(Your_Participantid) %>%
#   summarise(across(everything(), funs(100*mean(is.na(.)))))
...
To avoid the dimensionality curse we reduce the extracted features using a principal component analysis.
...
# corrplot::corrplot(var$contrib[,1:10], is.corr=FALSE) 
fviz_contrib(res.pca, choice = "var", axes = 1, top = 50)
fviz_contrib(res.pca, choice = "var", axes = 2, top = 50)

res.desc <-
  FactoMineR::dimdesc(
    FactoMineR::PCA(scaled_data, scale.unit = FALSE, graph = FALSE),
    axes = c(1, 2),
    proba = 0.05
  )
...
pca.out <- as.data.frame(res.pca$x[,1:3])

# load libraries
library(factoextra)
library(NbClust)
# elbow method
fviz_nbclust(pca.out, kmeans, method = "wss") +
  geom_vline(xintercept = 2, linetype = 2)+
  labs(subtitle = "Elbow method")
# Silhouette method
fviz_nbclust(pca.out, kmeans, method = "silhouette")+
...
eps_plot = kNNdistplot(pca.out, k=3)
# to draw an optimum line
eps_plot %>% abline(h = 3.5, lty = 2)


potentialMinPts <- round(SciViews::ln(nrow(pca.out)))
eps <- 3.5
minPts <- 2
...
The main clustering procedure, uses the features calculated for the relevant variables across all three studies. For our main method, we use the centroid-based k-means clustering. The procedure needs the researcher to make a selection on the optimal number of clusters in order to run the main algorithm. We use the Elbow and the Silhouette method, which both suggest two clusters to be the optimal solution.
...
raw_data <- featData
z_data <- featFullImp$featuresImpZ %>% select(-ends_with(featureExclusion))
...
IMPORTANT FIX: https://stackoverflow.com/questions/66448838/add-an-interactive-button-to-switch-which-variable-is-mapped-to-fill-in-ggplotly
...
```{r time-series linear trends by variable and cluster}
data.frame(ID = as.numeric(names(kmeansOut$cluster)), cluster = kmeansOut$cluster) %>%
  merge(., raw_data) %>%
  select(
    ID,
...
<!-- scaled_data <- featFullImp$featuresImpZMat %>% select(-ends_with(featureExclusion)) -->
<!-- nMClust <- list() -->
<!-- for(f in unique(gsub("^.*?\\_", "", names(scaled_data)))){ -->
...
<!-- scaled_data <- featFullImp$featuresImpZMat %>% select(-ends_with(featureExclusion))  # %>% select(starts_with(varNamS123NoInt))#%>% -->
