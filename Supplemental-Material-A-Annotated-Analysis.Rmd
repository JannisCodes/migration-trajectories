...
dtMiS2 <- 
  ...
  select(PID,
         ...
         week,
         everything()) %>%
  select_if(function(x) is.character(x) | (is.numeric(x) && sd(x) > 0)) %>%
         #which(sapply(. %>% select(where(is.numeric)), sd, na.rm = TRUE) > 0)) %>% # only vars that sd != 0
         #) %>% # only include variables that vary (i.e., sd != 0)
         #which(sd(., na.rm = TRUE) > 0)) %>% # only include variables that vary (i.e., sd != 0)
  as.data.frame 
