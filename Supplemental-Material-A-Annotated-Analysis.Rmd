...
varNamInMiMainPsbl <- c(
  paste(varNamInMiMain, rep(".pre", length(varNamInMiMain)), sep = ""),
  varNamInMiMain,
  paste(varNamInMiMain, rep(".post", length(varNamInMiMain)), sep = "")
)
...
varNamInMiS1Psbl <- c(
  paste(varNamInMiS1, rep(".pre", length(varNamInMiS1)), sep = ""),
  varNamInMiS1,
  paste(varNamInMiS1, rep(".post", length(varNamInMiS1)), sep = "")
)
...
dtMiS1 <- dtS1Red %>%
  ...
  which(!is.numeric(dtMiS1) | sapply(dtMiS1, sd, na.rm=TRUE)>0)
  
  select_if(~sd(., na.rm = TRUE) > 0) # only include variables that vary (i.e., sd != 0)
...
write.amelia(MiS1, file.stem = "data/imputed/S1Imputed.Amelia")
```
...
#   ) %>%
#   mutate(
#     variable = paste(gsub("_gmc", "", variable), TIDnum, sep = "_")
#   ) %>%
#   #select(-TIDnum) %>% 
#   arrange(variable, ID) %>%
#   mutate(
#     idVar = paste(ID, variable, sep = "_")
#   ) 
# plyr::join(test, data.frame(table(idVar = test$idVar))) 
...
We begin by plotting the item means of all variables. We plot the means of the individual variables for each measurement occasion, summarizing the responses of all participants at any given time point. The displayed time axis is the measurement index rather than the actual date to ensure comparability of the changes between each measurement. We plot the means for bi-daily, daily, and weekly timescales. 
