...
varNamS123MI <- c(varNamCore, varNamS123Aux)
...
capture.output(
  threeWayAnoDailyImp <- threewayanova(
    ...
    m = length(unique(dtPcaS123DailyImp$TIDnum)), # Nr. time points (B)
    p = length(names(dtPcaS123DailyImp)[!names(dtPcaS123DailyImp) %in% idVars]) # Nr. variables (C)
  ), 
file='NUL')
...
capture.output(
  threeWayAnoWeeklyImp <- threewayanova(
    ...
    m = length(unique(dtPcaS123WeeklyImp$TIDnum)), # Nr. time points (B)
    p = length(names(dtPcaS123WeeklyImp)[!names(dtPcaS123WeeklyImp) %in% idVars]) # Nr. variables (C)
  ), 
file='NUL')
