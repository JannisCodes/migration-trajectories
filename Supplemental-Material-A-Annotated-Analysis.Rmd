...
In progress. Weekly might not be possible with all aux variables (i.e., too many variables for too few observations).
...
dtMiS1Daily <- dtS1Red %>% 
  ...
  ungroup %>% 
  group_by(TIDnum) %>%
  mutate(across(c("date", "week"),  ~ replace_na(.x, calc_mode(.x)))) %>%
  ungroup %>% 
  mutate(across(ends_with(c(".pre", ".post")), ~ ifelse(is.nan(.), NA, .))) %>% 
  select(PID,
         TIDnum,
...
lares::corr_cross(
  dtMiS1Daily %>% select_if(is.numeric), # name of dataset
  max_pvalue = 0.05, # display only significant correlations (at 5% level)
  top = 20 # display top 20 couples of variables (by correlation coefficient)
)

lares::corr_cross(
  dtMiS1Weekly %>% select_if(is.numeric), # name of dataset
  max_pvalue = 0.05, # display only significant correlations (at 5% level)
  top = 20 # display top 20 couples of variables (by correlation coefficient)
)

# Missingness Percentage
# (colMeans(is.na(dtMiS1Weekly)))*100
...
MiS1Daily <-
  amelia(
    ...
    ts = "TIDnum",
    cs = "PID",
    idvars = c("date", "week", "IntergroupContact", "IngroupContact"),
    # lags = names(dtMiS1)[!names(dtMiS1) %in% idVars],
    # lags = names(dtMiS1)[names(dtMiS1) %in% varNamS1MiPsbl], # alternative?
    # leads = ...,
    # noms = names(dtMiS1)[grepl(paste(varNamdtS1Noms, collapse = "|^"), names(dtMiS1))],
...
save(MiS1Daily, file = "data/imputed/S1DailyImputed.RData")
load("data/imputed/S1DailyImputed.RData")
...
MiS1Weekly <-
  amelia(
    ...
    ts = "TIDnum",
    cs = "PID",
    idvars = c("week", "IntergroupContact", "IngroupContact"),
    # lags = names(dtMiS1)[!names(dtMiS1) %in% idVars],
    # lags = names(dtMiS1)[names(dtMiS1) %in% varNamS1MiPsbl], # alternative?
    # leads = ...,
    # noms = names(dtMiS1)[grepl(paste(varNamdtS1Noms, collapse = "|^"), names(dtMiS1))],
...
save(MiS1Weekly, file = "data/imputed/S1WeeklyImputed.RData")
load("data/imputed/S1WeeklyImputed.RData")
```
...
dtPcaS123Imp <- MiMain$imputations[[1]] %>% 
  ...
  arrange(ID, TIDnum) %>%
  filter(TIDnum <= min(max(dtS1Red$TIDnum), max(dtS2Red$TIDnum), max(dtS3Red$TIDnum)))
dtPcaS123DailyImp <- MiMainDaily$imputations[[1]] %>%
...
dtPcaS123WeeklyImp <- MiMainWeekly$imputations[[1]] %>%
...
dtMainDailyImpWide <- dtPcaS123DailyImp %>%
  select(
    -date
  ) %>% 
  melt(
...
dtMainWeeklyImpWide <- dtPcaS123WeeklyImp %>%
  select(
    -week
  ) %>% 
  melt(
...
capture.output(
  threeWayAnoBiDailyImp <- threewayanova(
    ...
    m = length(names(dtPcaS123Imp)[!names(dtPcaS123Imp) %in% idVars]), # Nr. variables (B)
    p = length(unique(dtPcaS123Imp$TIDnum)) # Nr. time points (C)
  ), 
file='NUL')
...
capture.output(
  threeWayAnoDailyImp <- threewayanova(
    ...
    m = length(names(dtPcaS123DailyImp)[!names(dtPcaS123DailyImp) %in% idVars]), # Nr. variables (B)
    p = length(unique(dtPcaS123DailyImp$TIDnum)) # Nr. time points (C)
  ), 
file='NUL')
...
capture.output(
  threeWayAnoWeeklyImp <- threewayanova(
    ...
    m = length(names(dtPcaS123WeeklyImp)[!names(dtPcaS123WeeklyImp) %in% idVars]), # Nr. variables (B)
    p = length(unique(dtPcaS123WeeklyImp$TIDnum)) # Nr. time points (C)
  ), 
file='NUL')
...
However, with the aggregation to `r length(unique(dtPcaS123WeeklyImp$TIDnum))` weekly measurements, the $Person*Time*Variable+error$ sums of squares become less prominent and the variable differences explain substantially more variance.
...
plot(t3MainRot$C[, 1], type = "n", ylim = range(t3MainRot$C), xlab = "Time occasion", ylab = "Component score")
points(t3MainRot$C[, 1], pch = 16)
lines(t3MainRot$C[, 1])
points(t3MainRot$C[, 2], pch = 17)
lines(t3MainRot$C[, 2])
legend("topleft", legend = c("C1", "C2"), pch = c(16, 17), bty = "n")
...
# lares::corr_cross(
#   dtMainImpWideCZ, # name of dataset
#   max_pvalue = 0.05, # display only significant correlations (at 5% level)
#   top = 50 # display top 50 couples of variables (by correlation coefficient)
# )
