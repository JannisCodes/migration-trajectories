...
# For checking
# validation <- rbind(
#   dtS1Red %>% select(any_of(idVars),any_of(validationNam)) %>% mutate(study = "S1"),
#   dtS2Red %>% select(any_of(idVars),any_of(validationNam)) %>% mutate(study = "S2"),
#   dtS3Red %>% select(any_of(idVars),any_of(validationNam)) %>% mutate(study = "S3")
# ) %>%
#   mutate(
#     IntGrAnx.pre = rowMeans(across(starts_with("IntGrAnx"))),
#     SWL.pre = rowMeans(across(starts_with("SWL")))
#   ) %>% 
#   group_by(study, PID) %>%
#   mutate(ID = cur_group_id()) %>%
#   ungroup
