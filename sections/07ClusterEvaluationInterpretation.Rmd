...
results$ar01$AttitudesDutch$conf.int
...
results_df <- results_df %>%
      mutate(star = ifelse(p_value < 0.001, "***", ifelse(p_value < 0.01, "**", ifelse(p_value < 0.05, "*", ""))))
...
for (feature in unique(results_df$feature)) {
  ...
  # Create a matrix for the forestplot function
  plot_matrix <- rbind(c("", "Difference", "Lower CI", "Upper CI"),
                       cbind(feature_data$concept,
                             feature_data$difference,
                             feature_data$lwr,
                             feature_data$upr))

  # Create the forest plot
  feature_plot_list[[feature]] <- forestplot::forestplot(labeltext = plot_matrix[, 1], 
             mean = as.numeric(plot_matrix[, 2]),
             lower = as.numeric(plot_matrix[, 3]),
             upper = as.numeric(plot_matrix[, 4]),
             xlab = "Difference",
             title = paste0("Forest plot for ", feature))
}

concept_plot_list <- list()
for (concept in unique(results_df$concept)) {
  # Subset the data for the current feature
  ...
  # Create the forest plot
  concept_plot_list[[concept]] <- forestplot::forestplot(labeltext = plot_matrix[, 1], 
             mean = as.numeric(plot_matrix[, 2]),
             lower = as.numeric(plot_matrix[, 3]),
             upper = as.numeric(plot_matrix[, 4]),
             xlab = "Difference",
             title = paste0("Forest plot for ", concept))
}
